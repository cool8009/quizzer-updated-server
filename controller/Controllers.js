module.exports = {
    AnswerController: require('./ControllersFolder/AnswerController'),
    FieldOfStudyController: require('./ControllersFolder/FieldOfStudyController'),
    QuestionController: require('./ControllersFolder/QuestionController'),
    TestController: require('./ControllersFolder/TestController'),
    TagController: require('./ControllersFolder/TagController'),
    CompanyController: require('./ControllersFolder/CompanyController'),
    TestInstanceController: require('./ControllersFolder/TestInstanceController'),
    AnswerInstanceController: require('./ControllersFolder/AnswerInstanceController'),
    EmailController: require('./ControllersFolder/EmailController')
}